	...
	M.init=function(div)
	{
		...
		M.castSpell=function(spell,obj)
		{
			...
			if (out!=-1)
			{
				...
				if (fail) PlaySound('assets/sounds/spellFail.mp3',0.75); else PlaySound('assets/sounds/spell.mp3',0.75);
				return true;
			}
		...
		var str='';
		str+='<style>'+
		'#grimoireBG{background:url('+Game.resPath+'assets/images/shadedBorders.png),url('+Game.resPath+'assets/images/BGgrimoire.jpg);background-size:100% 100%,auto;position:absolute;left:0px;right:0px;top:0px;bottom:16px;}'+
		...
		'.grimoireSpell{box-shadow:4px 4px 4px #000;cursor:pointer;position:relative;color:#f33;opacity:0.8;text-shadow:0px 0px 4px #000,0px 0px 6px #000;font-weight:bold;font-size:12px;display:inline-block;width:60px;height:74px;background:url('+Game.resPath+'assets/images/spellBG.png);}'+
		...
		{
			var me=M.spells[i];
			AddEvent(l('grimoireSpell'+me.id),'click',function(spell){return function(){PlaySound('assets/sounds/tick.mp3');M.castSpell(spell);}}(me));
		}
