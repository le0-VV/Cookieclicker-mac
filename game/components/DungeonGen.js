...
var DungeonGen=function()
{
	...
	this.Map.prototype.drawDetailed=function()
	{
		...
		for (var y=0;y<this.h;y++){for (var x=0;x<this.w;x++){
				...
				str+='<div style="opacity:'+opacity+';width:'+size+'px;height:'+size+'px;position:absolute;left:'+(x*size)+'px;top:'+(y*size)+'px;display:block;padding:0px;margin:0px;background:#'+colors[this.data[x][y][0]]+' url(assets/images/dungeonTiles.png) '+(-pic[0]*16)+'px '+(-pic[1]*16)+'px;color:#999;" title="'+title+'"></div>';
			}
			str+='<br>';
		}
