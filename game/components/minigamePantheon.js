	...
	M.init=function(div)
	{
		...
		M.dropGod=function()
		{
			...
			if (M.slotHovered!=-1 && (M.swaps==0 || M.dragging.slot==M.slotHovered))//dropping on a slot but no swaps left, or slot is the same as the original
			{
				...
				PlaySound('assets/sounds/sell1.mp3',0.75);
			}
			else if (M.slotHovered!=-1)//dropping on a slot
			{
				...
				PlaySound('assets/sounds/tick.mp3');
				PlaySound('assets/sounds/spirit.mp3',0.5);
		...
		M.hoverSlot=function(what)
		{
			...
			if (M.dragging)
			{
				...
				PlaySound('assets/sounds/clickb'+Math.floor(Math.random()*7+1)+'.mp3',0.75);
			}
		}
		...
		var str='';
		str+='<style>'+
		'#templeBG{background:url('+Game.resPath+'assets/images/shadedBorders.png),url('+Game.resPath+'assets/images/BGpantheon.jpg);background-size:100% 100%,auto;position:absolute;left:0px;right:0px;top:0px;bottom:16px;}'+
		...
		'.templeGod{box-shadow:4px 4px 4px #000;cursor:pointer;position:relative;color:#f33;opacity:0.8;text-shadow:0px 0px 4px #000,0px 0px 6px #000;font-weight:bold;font-size:12px;display:inline-block;width:60px;height:74px;background:url('+Game.resPath+'assets/images/spellBG.png);}'+
